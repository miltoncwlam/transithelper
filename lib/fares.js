...
export function fareFields(row, extra = {}) {
  if (!row) return {};
  const sectionPrices = Array.isArray(row.section_prices)
    ? row.section_prices.map(Number).filter((n) => Number.isFinite(n))
    : [];
  ...
  return {
    ...
    section_prices: sectionPrices.length > 1 ? sectionPrices : undefined,
    section_fare_hkd: section
  };
}
...
export async function fareForRoute(route, onSeq, offSeq) {
  ...
  return {
    ...
    ...fareFields(row, { on_seq: onSeq, off_seq: offSeq })
  };
}
