  ...
  zh: {
    ...
    sections: [
      ...
      },
      {
        h: '到站、終點、地圖與各站時間',
        p: [
          '選路線後，用可搜尋的車站名單或點地圖站號選上車站。可再選此程終點；終點只會列出上車之後的站。終點可留空，此時會跟車至路線最後一站。沒有上次巴士時，到站頁會先用定位列出附近站柱現正公布的班次；也可再按「附近到站」。儲存為回家附近或返工附近後仍留在到站頁；每種只保留一處。之後可按到站頁的「回家附近」／「返工附近」重開該位置的實時牌，不必再搜。開啟時沒有公布就會留空。不會在啟動時改去「我的回家路線」。',
