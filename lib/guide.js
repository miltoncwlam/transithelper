    ...
    sections: [
      {
        ...
        p: [
          ...
          '未選路線時可按「附近到站」用定位顯示附近站柱現正公布的巴士與小巴。沒有公布就會留空。可把該位置儲存為回家附近或返工附近。',
          '選好路線後，用地圖或可搜尋的車站名單選上車站，即顯示該站實時到站。終點可留空，此時會跟車至路線最後一站。地圖畫在本頁，是運輸署公布走線。右上角可把時間改為鐘面或倒數。'
        ]
      },
      {
        h: '轉乘助手',
      ...
      },
      {
        h: '趕車助手',
        p: [
          '誤了這一站、或這一班剛開出時，按「趕這一班」。只跟鎖定的那班車：若後面車站仍有這一班的實時（或由最近一站實時推算並標「估計」），會比較步行與到站。步行一律標估計。',
    ...
    sections: [
      {
        ...
        p: [
          ...
          'Before picking a route, Nearby arrivals uses your location to list live buses and minibuses at poles around you. Empty operator feeds stay empty. You can save that place as home nearby or work nearby.',
          'After a route, pick a boarding stop from the searchable list or the map. You may pick a destination on the same trip; leave it empty to follow to the last stop. The official path is drawn on this tab. Use the header to switch clock times and countdown.'
        ]
      },
      {
        h: 'Transfer Buddy',
      ...
      },
      {
        h: 'Catch-up helper',
        p: [
          'If you missed this pole, or the bus just left, tap Catch this bus. The app stays on the locked trip: if that same vehicle is still live further along (or a hop from the last live match, marked as an estimate), it compares your walk with the bus clock. Walk times are always estimates.',
  ...
  zh: {
    ...
    sections: [
      ...
      },
      {
        h: '到站、終點、地圖與各站時間',
        p: [
          '選路線後，用可搜尋的車站名單或點地圖站號選上車站。可再選此程終點；終點只會列出上車之後的站。終點可留空，此時會跟車至路線最後一站。未選路線時可按「附近到站」用定位列出附近站柱現正公布的班次。',
      ...
      },
      {
        h: '趕車助手與錯過代價',
        p: [
          '誤了上車站、或巴士剛開出時，按「趕這一班」。趕車助手只跟鎖定的那班車，用後面車站的實時到站（或由最近一站實時推算並標「估計」）比較步行時間與到站時間。步行一律標估計。沒有後面車站的實時（也沒有可跟的實時錨點）就不會顯示趕車列。',
  ...
  en: {
    ...
    sections: [
      ...
      },
      {
        ...
        p: [
          'If you missed the boarding stop or the bus just left, tap Catch this bus. Catch-up helper stays on the locked trip and compares a walk to a later pole with that vehicle’s live clock (or a hop from the last live match, marked as an estimate). Walk times are always estimates. No downstream live match and no live anchor means no catch-up row.',
