...
function toKeep(query, info) {
  ...
  const idle = info.filter((z) => !z.live.length && (z.seq.length || ['KMB', 'LWB', 'CTB', 'NLB', 'GMB', 'MTRB'].includes(serviceCo(z.x))));
...
export function keepFromServices(query, services) {
  return toKeep(query, (services || []).filter(Boolean).map((row) => ({ x: row, live: [], seq: [{}] })));
}

export async function searchLive(cache, directory, routeStr, opts = {}) {
  const q = n(routeStr);
  if (!q) return { error: 'noRoute', keep: [], auto: null };
  ...
  const gmbMs = needGmb ? (haveFranchised ? 4000 : 8000) : 0;
  const lookup = needGmb
    ? (opts.gmbPromise ? Promise.resolve(opts.gmbPromise).catch(() => []) : gmbLookup(cache, q).catch(() => []))
    : Promise.resolve([]);
  const [kmbInfo, gmbRows] = await Promise.all([
    kmbish.length ? probeKmbBatch(cache, kmbish) : Promise.resolve([]),
    gmbMs ? withBudget(lookup, gmbMs) : Promise.resolve([])
  ]);
  const extraGmb = [];
