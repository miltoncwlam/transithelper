...
import { citybusRouteStops, citybusStopEta, citybusStopEtas, stopCompany } from './citybus.js';
...
async function routeStops(cache, stopMap, service) {
  if (serviceCompany(service) === 'GMB') {
    const rows = await gmbRouteStops(cache, service);
    addStops(rows);
    return rows;
  }
  if ((service.co || 'KMB') === 'CTB') {
...
async function firstRouteEtaTables(cache, first, seq, fromIdx, toIdx) {
  ...
  if (firstCo === 'GMB' || firstCo === 'CTB') {
    ...
    const lists = await mapPool(slice, 6, (row) => (
      firstCo === 'GMB'
        ? gmbStopEta(cache, row, first)
        : citybusStopEta(cache, row.stop, first.route)
    ));
...
async function etasAtStops(cache, stops, routes) {
  const lists = await mapPool(stops, 8, (stop) => (
    stopCompany(stop) === 'CTB'
      ? citybusStopEtas(cache, stop.stop)
  ...
  stops.forEach((stop, i) => {
    for (const eta of lists[i] || []) {
      if (!eta.eta) continue;
      rows.push({ eta: enrichEta({ ...eta, co: eta.co || stopCompany(stop) }, routes), stop });
    }
  });
  return rows;
}
...
function groupCandidates(live) {
  ...
  for (const row of live) {
    ...
    const key = [co, row.eta.route, row.eta.dir, row.eta.service_type || '1'].join('|');
...
function publicItem(item) {
  const { fromStop, fromSeq, fromLat, fromLng, ...rest } = item;
  return rest;
}
...
async function plan(cache, stopMap, allStops, body, routes) {
  ...
  const needChain = phase === 'connections' && first && boardIdx >= 0 && interIdx >= boardIdx;
  ...
  if (phase === 'departures') {
    ...
    const departures = boardFirstTimes.map((eta) => ({ eta, dest, route: first?.route || '' }));
    const directs = [];
    ...
    candidateList.forEach((candidate, idx) => {
      ...
      for (const entry of candidate.entries) {
        ...
        if (!kind) continue;
        addUnique(directs, connectionRow(kind, candidate, entry, match, { waitAfterFirstMinutes: null }), firstAlightIds);
      }
    });

    directs.sort((a, b) => new Date(a.eta) - new Date(b.eta));
    const withDiscounts = first ? await attachDiscounts(directs.map(publicItem), first) : directs.map(publicItem);
    return {
      phase: 'departures',
  ...
  const allPublic = list.map(publicItem);
  const ranked = pickConnections(list).map(publicItem);
