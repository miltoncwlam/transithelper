import { fareForRoute } from '../00-required/fares.js';
...
async function loadWaypoint(id, dir) {
  const key = `${id}-${dir}`;
  if (waypointCache.has(key)) return waypointCache.get(key);
  const urls = FILES.map((make) => make(id, dir));
  const json = await Promise.any(urls.map((url) => fetchJson(url)));
  waypointCache.set(key, json);
  return json;
}
...
async function officialRoutePathWork({ route, co, bound, orig, dest, stops, td_route_id } = {}) {
...
export async function officialRoutePath(opts = {}) {
  return Promise.race([
    officialRoutePathWork(opts),
