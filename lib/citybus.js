...
export async function citybusStop(cache, stopId) {
  try {
    ...
    if (!row || !row.stop) {
      return { stop: stopId, name_tc: stopId, name_en: stopId, lat: null, long: null, co: 'CTB' };
    }
    return {
      stop: row.stop,
...
export async function citybusRouteStops(cache, service, stopMap) {
  ...
  try {
    ...
    return mapPool(seq, 8, async (row) => {
      const known = stopMap?.get(row.stop);
      const meta = known?.name_tc ? known : await citybusStop(cache, row.stop);
      return {
        ...row,
        ...meta,
        stop: row.stop,
