...
export function mergeJourneyGroups(options) {
  ...
  for (const members of buckets.values()) {
    ...
    groups.push({
      ...
      best,
      members: sorted,
      laterEtas: etas.filter((eta) => eta !== best.eta),
  ...
  const streetBest = groups[0];
  return groups.map((group, i) => {
    const slower = streetBest?.best?.arrive && group.best.arrive
      ? Math.max(0, Math.round((new Date(group.best.arrive) - new Date(streetBest.best.arrive)) / 60000))
      : 0;
    return {
      ...group,
      recommended: i === 0,
