...
export function pathBetween(line, origin, dest, trainDest) {
  const oCode = String(origin || '').toUpperCase();
  const dCode = String(dest || '').toUpperCase();
  const tCode = String(trainDest || '').toUpperCase();
  for (const path of lineRoutes(line)) {
...
export async function planMtrRide(line, sta, dest) {
  const keys = [...new Set([line, ...linesServing(sta, dest)])];
  const memo = new Map();
