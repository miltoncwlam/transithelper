...
import { citybusAllStops, citybusRoutes } from '../00-required/citybus.js';
import { startGtfsLoad } from '../00-required/gtfs.js';
import { gmbHydrateServices, gmbRoutes } from '../00-required/gmb.js';
...
function startBackgroundHydrate() {
  ...
  if (!directory.routes.some((row) => row.co === 'GMB' && row.gmb_route_id)) {
    hydrateGmb().catch(() => {});
  }
}
