...
import { gmbAllStops, gmbHydrateServices, gmbRoutes } from './gmb.js';
...
async function hydrateGmb() {
  ...
  hydratingGmb = (async () => {
    try {
      const services = await gmbHydrateServices(cache);
      if (services.length) {
        setDirectory({
      ...
      const stops = await gmbAllStops(cache, services.length ? services : directory.routes.filter((row) => row.co === 'GMB'));
      if (stops.length) {
        setDirectory({
          routes: directory.routes,
          stops: mergeStops(directory.stops, stops)
...
function startBackgroundHydrate() {
  ...
  if (!directory.routes.some((r) => r.co === 'GMB' && r.gmb_route_id) || !directory.stops.some((s) => s.co === 'GMB')) {
    hydrateGmb().catch(() => {});
  }
}
...
export async function ensureNearbyStops() {
  const dir = await getDirectory();
  const jobs = [];
  if (!dir.stops.some((stop) => stop.co === 'CTB')) jobs.push(hydrateCitybusStops(dir.routes, dir.stops.filter((s) => s.co !== 'CTB')));
  if (!dir.stops.some((stop) => stop.co === 'NLB')) jobs.push(hydrateNlb(dir.routes));
  await Promise.all(jobs.map((job) => job.catch(() => directory)));
  if (!directory.stops.some((stop) => stop.co === 'GMB')) hydrateGmb().catch(() => {});
  return directory;
}
...
export function addStops(rows) {
  ...
  for (const row of rows || []) {
    ...
    if (stopNameMissing(stop) && stop.co === 'CTB') continue;
