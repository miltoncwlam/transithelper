import { createCache } from '../00-required/cache.js';
import { attachFaresToRoutes } from '../00-required/fares.js';
import { kmbFetch } from '../00-required/kmb.js';
import { citybusStopCatalog, citybusRoutes } from '../00-required/citybus.js';
import { startGtfsLoad } from '../00-required/gtfs.js';
import { gmbHydrateServices, gmbRoutes, gmbAllStops } from '../00-required/gmb.js';
import { nlbAllStops, nlbRoutes } from '../00-required/nlb.js';
import { mtrBusDirectoryRoutes } from '../00-required/mtrbus.js';
import { stopNameMissing } from '../00-required/stopName.js';
import { bindAddStops } from '../00-required/addStops.js';
import { warmTopology } from '../00-required/topology.js';
import { readSnapshot, writeSnapshot } from './snapshotStore.js';
import { readFileSync, writeFileSync } from 'node:fs';
import os from 'node:os';
import path from 'node:path';

const DIR_TTL = 12 * 60 * 60 * 1000;
const CACHE_VERSION = 5;
const FILE = 'transitbuddy-directory.json';
const GMB_HTTP_FILE = path.join(os.tmpdir(), 'transitbuddy-gmb-http.json');

const mem = createCache();

function loadGmbHttp() {
  try {
    const raw = JSON.parse(readFileSync(GMB_HTTP_FILE, 'utf8'));
    const now = Date.now();
    for (const [key, hit] of Object.entries(raw.entries || {})) {
      if (!key.startsWith('gmb:') || !hit || hit.exp <= now) continue;
      mem.set(key, hit.val, hit.exp - now);
    }
  } catch {}
}

function saveGmbHttp(key, val, ttlMs) {
  if (!String(key).startsWith('gmb:') || ttlMs < 60_000) return;
  try {
    let entries = {};
    try {
      entries = JSON.parse(readFileSync(GMB_HTTP_FILE, 'utf8')).entries || {};
    } catch {}
    entries[key] = { val, exp: Date.now() + ttlMs };
    writeFileSync(GMB_HTTP_FILE, JSON.stringify({ entries }));
  } catch {}
}

loadGmbHttp();

export const cache = {
  get: (key) => mem.get(key),
  set: (key, val, ttlMs) => {
    mem.set(key, val, ttlMs);
    saveGmbHttp(key, val, ttlMs);
    return val;
  }
};

let directory = { routes: [], stops: [], stopMap: new Map() };
let loading = null;
let hydratingCtb = null;
let hydratingNlb = null;
let hydratingGmb = null;

function buildStopMap(stops) {
  const map = new Map();
  for (const stop of stops || []) {
    if (!stop?.stop) continue;
    map.set(stopKey(stop), stop);
    if (!map.has(stop.stop)) map.set(String(stop.stop), stop);
  }
  return map;
}

function revive(raw) {
  const stops = (raw.stops || []).map((stop) => ({ ...stop, co: stop.co || 'KMB' }));
  return {
    routes: mergeRoutes(raw.routes || [], mtrBusDirectoryRoutes()),
    stops,
    stopMap: buildStopMap(stops)
  };
}

export function stopKey(stop) {
  const co = String(stop?.co || 'KMB').toUpperCase();
  const id = stop?.stop || stop;
  return `${co}:${id}`;
}

async function readFileCache() {
  const raw = await readSnapshot(FILE);
  if (!raw || raw.version !== CACHE_VERSION) return null;
  if (Date.now() - (raw.savedAt || 0) < DIR_TTL && raw.routes?.length) return revive(raw);
  return null;
}

async function writeFileCache(dir, opts = {}) {
  try {
    await writeSnapshot(FILE, {
      version: CACHE_VERSION,
      savedAt: Date.now(),
      routes: dir.routes,
      stops: dir.stops
    }, opts);
  } catch {}
}

function setDirectory(next) {
  directory = {
    ...next,
    stopMap: buildStopMap(next.stops || [])
  };
  return directory;
}

function mergeStops(existing, extra) {
  const map = new Map((existing || []).map((stop) => [stopKey(stop), stop]));
  for (const stop of extra || []) {
    if (!stop?.stop) continue;
    const key = stopKey(stop);
    if (!map.has(key)) map.set(key, stop);
  }
  return [...map.values()];
}

function mergeRoutes(existing, extra, replaceCos = []) {
  const drop = new Set(replaceCos.map((c) => String(c).toUpperCase()));
  const kept = drop.size ? (existing || []).filter((row) => !drop.has(String(row.co || 'KMB').toUpperCase())) : (existing || []);
  const seen = new Set();
  const out = [];
  for (const row of [...kept, ...(extra || [])]) {
    const key = [
      String(row.co || 'KMB').toUpperCase(),
      row.route,
      row.bound,
      row.service_type,
      row.gmb_route_id || '',
      row.nlb_route_id || '',
      row.orig_en,
      row.dest_en
    ].join('|');
    if (seen.has(key)) continue;
    seen.add(key);
    out.push(row);
  }
  return out;
}

async function hydrateCitybusStops(routes, baseStops) {
  if (hydratingCtb) return hydratingCtb;
  hydratingCtb = (async () => {
    try {
      const ctbStops = await citybusStopCatalog(cache);
      if (!ctbStops.length) return directory;
      setDirectory({
        routes: directory.routes.length ? directory.routes : routes,
        stops: mergeStops(baseStops || directory.stops.filter((s) => s.co !== 'CTB'), ctbStops)
      });
      await writeFileCache(directory);
      return directory;
    } finally {
      hydratingCtb = null;
    }
  })();
  return hydratingCtb;
}

async function hydrateNlb(routes) {
  if (hydratingNlb) return hydratingNlb;
  hydratingNlb = (async () => {
    try {
      const nlb = routes.filter((row) => row.co === 'NLB');
      const stops = await nlbAllStops(cache, nlb);
      if (!stops.length) return directory;
      setDirectory({
        routes: directory.routes,
        stops: mergeStops(directory.stops, stops)
      });
      await writeFileCache(directory);
      return directory;
    } finally {
      hydratingNlb = null;
    }
  })();
  return hydratingNlb;
}

async function hydrateGmb() {
  if (hydratingGmb) return hydratingGmb;
  hydratingGmb = (async () => {
    try {
      const services = await gmbHydrateServices(cache, { limit: 120 });
      if (services.length) {
        const gmbStops = await gmbAllStops(cache, services.slice(0, 80)).catch(() => []);
        const kept = directory.routes.filter((row) => String(row.co || '').toUpperCase() !== 'GMB' || row.gmb_route_id);
        setDirectory({
          routes: mergeRoutes(kept, services),
          stops: mergeStops(directory.stops, gmbStops)
        });
        await writeFileCache(directory);
      }
      return directory;
    } finally {
      hydratingGmb = null;
    }
  })();
  return hydratingGmb;
}

function startBackgroundHydrate() {
  if (!directory.stops.some((s) => s.co === 'NLB')) hydrateNlb(directory.routes).catch(() => {});
  if (!directory.stops.some((s) => s.co === 'CTB')) {
    hydrateCitybusStops(directory.routes, directory.stops.filter((s) => s.co !== 'CTB')).catch(() => {});
  }
  if (!directory.routes.some((row) => row.co === 'GMB')) {
    gmbRoutes(cache).then((gmb) => {
      if (!gmb.length) return;
      setDirectory({ routes: mergeRoutes(directory.routes, gmb), stops: directory.stops });
    }).catch(() => {});
  }
  if (!directory.routes.some((row) => row.co === 'GMB' && row.gmb_route_id) || !directory.stops.some((s) => s.co === 'GMB')) {
    hydrateGmb().catch(() => {});
  }
}

export function rememberGmbServices(services) {
  const rows = (services || []).filter((row) => row?.gmb_route_id);
  if (!rows.length) return;
  setDirectory({
    routes: mergeRoutes(directory.routes, rows),
    stops: directory.stops
  });
}

async function loadDirectory() {
  const fromDisk = await readFileCache();
  if (fromDisk) {
    setDirectory(fromDisk);
    startBackgroundHydrate();
    return directory;
  }
  const [routes, stops, ctb, nlb, gmb] = await Promise.all([
    kmbFetch('/route/', cache, DIR_TTL),
    kmbFetch('/stop', cache, DIR_TTL),
    citybusRoutes(cache),
    nlbRoutes(cache),
    gmbRoutes(cache)
  ]);
  const kmbRoutes = (routes || []).map((row) => ({ ...row, co: row.co || 'KMB' }));
  const kmbStops = (stops || []).map((row) => ({ ...row, co: row.co || 'KMB' }));
  setDirectory({
    routes: [...kmbRoutes, ...ctb, ...nlb, ...gmb, ...mtrBusDirectoryRoutes()],
    stops: kmbStops
  });
  await writeFileCache(directory);
  startBackgroundHydrate();
  return directory;
}

let fareAttach = null;

async function withFares(dir) {
  if (!dir?.routes?.length) return dir;
  return {
    ...dir,
    routes: await attachFaresToRoutes(dir.routes)
  };
}

function attachFaresInBackground() {
  if (fareAttach || !directory.routes.length) return;
  if (directory.routes.some((row) => row.full_fare_hkd != null)) return;
  fareAttach = withFares(directory)
    .then((next) => {
      if (!next?.routes?.length || next.routes === directory.routes) return;
      setDirectory({ routes: next.routes, stops: directory.stops });
    })
    .catch(() => {})
    .finally(() => {
      fareAttach = null;
    });
}

function startLoad() {
  if (!loading) {
    loading = loadDirectory().catch((error) => {
      loading = null;
      throw error;
    });
  }
  return loading;
}

export function snapshotDirectory() {
  startGtfsLoad();
  if (directory.routes.length) {
    startLoad().catch(() => {});
    startBackgroundHydrate();
    return Promise.resolve(directory);
  }
  return readFileCache().then((fromDisk) => {
    if (fromDisk) setDirectory(fromDisk);
    startLoad().catch(() => {});
    startBackgroundHydrate();
    return directory;
  });
}

export async function getDirectoryFast(budgetMs = 3500) {
  startGtfsLoad();
  if (directory.routes.length && directory.stops.length) return directory;
  const fromDisk = await readFileCache();
  if (fromDisk) {
    setDirectory(fromDisk);
    startBackgroundHydrate();
    startLoad().catch(() => {});
    return directory;
  }
  const pending = startLoad();
  return Promise.race([
    pending.then(() => directory),
    new Promise((resolve) => setTimeout(() => resolve(directory), budgetMs))
  ]);
}

export async function getDirectory() {
  startGtfsLoad();
  if (directory.routes.length && directory.stops.length) {
    attachFaresInBackground();
    startBackgroundHydrate();
    return directory;
  }
  const dir = await startLoad();
  attachFaresInBackground();
  return dir;
}

export async function ensureCitybusStops() {
  return getDirectory();
}

export async function ensureNearbyStops() {
  return getDirectory();
}

export async function warmDirectory() {
  const dir = await getDirectory();
  await Promise.all([
    hydrateCitybusStops(dir.routes, dir.stops.filter((s) => s.co !== 'CTB')).catch(() => directory),
    hydrateNlb(dir.routes).catch(() => directory),
    hydrateGmb().catch(() => directory),
    startGtfsLoad(),
    warmTopology(cache, dir, { budgetMs: 45000, persistRemote: true }).catch(() => {})
  ]);
  await writeFileCache(directory, { remoteImmediately: true });
  return directory;
}

export function addStops(rows) {
  if (!directory.routes.length) return;
  let changed = false;
  for (const row of rows || []) {
    if (!row?.stop) continue;
    const key = stopKey(row);
    if (directory.stopMap.has(key)) continue;
    const stop = {
      stop: String(row.stop),
      name_tc: stopNameMissing(row) ? '' : (row.name_tc || row.name_en || ''),
      name_en: stopNameMissing(row) ? '' : (row.name_en || row.name_tc || ''),
      lat: row.lat,
      long: row.long,
      co: row.co || (String(row.stop).length === 6 ? 'CTB' : 'KMB')
    };
    directory.stops.push(stop);
    directory.stopMap.set(stopKey(stop), stop);
    if (!directory.stopMap.has(String(stop.stop))) directory.stopMap.set(String(stop.stop), stop);
    changed = true;
  }
  if (changed) writeFileCache(directory);
}

bindAddStops(addStops);
